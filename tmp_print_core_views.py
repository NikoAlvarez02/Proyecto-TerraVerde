...
def home_dashboard(request):
    ...
    # Todos los turnos programados hoy (cualquier hora)
    turnos_hoy = (
        turnos_hoy_qs
        .select_related('paciente')
        .order_by('fecha_hora')
...
class SessionLoginView(LoginView):
    """LoginView que persiste e informa intentos fallidos en la sesión.

    - Incrementa `request.session['login_fail_count']` en `form_invalid`.
    - Resetea el contador al validar credenciales en `form_valid`.
    - Expone en el contexto: `fail_count` y `fail_lock` (True si >= 5).
    """
    ...
    def set_fail_count(self, request, value):
        ...
        # Garantizar que la sesión se marque como modificada
